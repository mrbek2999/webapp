...
import {cartSave, messageReset} from "../../store/cart";
...
function ProductModal(props) {
    ...
    useEffect(() => {
        if(props.message == 'deleted'){
            ...
            toast.success('Товары добавлены в корзину', {theme: 'colored', draggable: false, autoClose:1600})
            props.messageReset()
            ...
            {props.orderItem ?
                <Modal show={show} onHide={handleClose} className={'product-order mt-3'} aria-labelledby="contained-modal-title-vcenter">
                    ...
                    <div className={'addToCartBlock'}>
                        ...
                        <button className={'addToCartBtn'} onClick={()=>addToCart(props.orderItem)}>Добавить в корзину</button>
                    </div>
...
export default connect(props => props.cart, {cartSave, messageReset})(ProductModal);